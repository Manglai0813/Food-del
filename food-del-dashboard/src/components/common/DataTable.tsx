        ...
        config.table.columns.forEach((colConfig) => {
            ...
            cols.push({
                ...
                cell: ({ getValue, row }) => {
                    ...
                    if (colConfig.render) {
                        return colConfig.render(value, row.original as Record<string, unknown>);
                    }
                ...
                cell: ({ row }) => (
                    <div className="flex justify-center">
                        <ActionMenu
                            ...
                            onAction={onActionRef.current as ((action: string, row: Record<string, unknown>) => void) | undefined}
                        />
                    </div>
                ),
                enableSorting: false,
...
function formatCellValue(value: unknown, type: string): React.ReactNode {
    ...
    switch (type) {
        ...
        case 'date':
            return new Date(value as string | number | Date).toLocaleDateString('ja-JP');

        case 'datetime':
            return new Date(value as string | number | Date).toLocaleString('ja-JP');
        ...
        case 'image':
            return value ? (
                <img src={value as string} alt="" className="w-12 h-12 object-cover rounded" />
            ) : '-';
