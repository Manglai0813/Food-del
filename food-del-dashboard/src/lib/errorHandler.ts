...
export const handleHttpError = (error: unknown): ApiError => {
        ...
        switch (status) {
                case HTTP_STATUS.BAD_REQUEST:
                        return new ApiError(
                                data?.message || data?.error || '不正なリクエストです',
                                status,
                                'BAD_REQUEST',
                                data?.details
                        );
                ...
                case HTTP_STATUS.UNAUTHORIZED:
                        return new ApiError(
                                data?.message || ERROR_MESSAGES.AUTH.INVALID_CREDENTIALS,
                                status,
                                'UNAUTHORIZED'
                        );
                ...
                case HTTP_STATUS.FORBIDDEN:
                        return new ApiError(
                                data?.message || ERROR_MESSAGES.AUTH.ACCESS_DENIED,
                                status,
                                'FORBIDDEN'
                        );
                ...
                case HTTP_STATUS.UNPROCESSABLE_ENTITY:
                        return new ApiError(
                                data?.message || data?.error || 'バリデーションエラー',
                                status,
                                'VALIDATION_ERROR',
                                data?.details
                        );
                ...
                case HTTP_STATUS.INTERNAL_SERVER_ERROR:
                        return new ApiError(
                                data?.message || ERROR_MESSAGES.SYSTEM.SERVER_ERROR,
                                status,
                                'SERVER_ERROR'
                        );
                ...
                default:
                        return new ApiError(
                                data?.message || data?.error || ERROR_MESSAGES.SYSTEM.UNKNOWN_ERROR,
                                status,
                                'UNKNOWN_ERROR'
                        );
        }
