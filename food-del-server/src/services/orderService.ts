...
/**
 * 注文ビジネスロジックサービス
 * 注文の作成、検索、更新、統計などの業務処理を提供します
 */
export class OrderService {

        // カートから注文を作成（トランザクション処理）
        static async createOrderFromCart(
                ...
                try {
                        return await prisma.$transaction(async (tx) => {
                                ...
                                const orderItems = await Promise.all(
                                        userCart.cart_items.map(async (item) => {
                                                ...
                                                // 注文アイテムを作成（価格スナップショット）
                                                return await tx.orderItem.create({
                                                        data: {
                                                                order_id: order.id,
                                                                food_id: item.food_id,
                                                                quantity: item.quantity,
                                                                price: item.food.price // 注文時の価格を保存
                                                        }
                                                });
                                        })
                                );

                                // カートをクリア（予約解除は不要：既に確定済み）
                        ...
                        throw new Error(`注文作成に失敗しました: ${error?.message || 'Unknown error'}`);
                }
        }
        ...
        static async cancelOrder(
                ...
                try {
                        ...
                        throw new Error(`注文キャンセルに失敗しました: ${error?.message || 'Unknown error'}`);
                }
        }
        ...
        static async updateOrderStatus(
                ...
                updatedByUserId: number = 1 // デフォルトは管理者ID（実際の実装では認証ユーザーIDを使用）
        ): Promise<OrderData | null> {
                return await prisma.$transaction(async (tx) => {
                        // 注文を取得
                        const order = await tx.order.findUnique({
        ...
        // 内部ヘルパー：完全な注文データを構築
        private static async buildOrderData(orderId: number, tx?: any): Promise<OrderData> {
        ...
        // 内部ヘルパー：OrderDataに変換
        private static transformToOrderData(order: any): OrderData {
