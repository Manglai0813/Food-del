...
// 敏感なヘッダー情報をフィルタリングする関数
function filterSensitiveHeaders(headers: Record<string, string | string[] | undefined>): Record<string, string | string[] | undefined> {
...
// 不審なリクエストパターンの検出とエラーハンドリングミドルウェア
export const requestControlErrorHandler = (error: unknown, req: Request, res: Response, next: NextFunction) => {
    // PayloadTooLargeError
    const errorStatus = (error && typeof error === 'object' && 'status' in error && typeof error.status === 'number') ? error.status : undefined;
    const errorCode = (error && typeof error === 'object' && 'code' in error && typeof error.code === 'string') ? error.code : undefined;
    if (errorStatus === 413 || errorCode === 'LIMIT_FILE_SIZE') {
